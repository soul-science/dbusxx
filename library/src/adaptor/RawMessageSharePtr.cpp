...
RawBusMessagePtr createMethodCall(RawBusPtr aBus, std::string_view aDestination,
    ...
    sd_bus_message_new_method_call(aBus, &raw,
        aDestination.empty() ? nullptr : aDestination.data(),
        aPath.data(), aInterface.data(), aMember.data());
    return raw;
}
...
std::string getSignature(RawBusMessagePtr aMsg, bool aComplete) {
    if (!aMsg) {
        return std::string();
    }
...
bool isEnd(RawBusMessagePtr aMsg, bool aComplete) {
    ...
    return sd_bus_message_at_end(aMsg, aComplete);
}

Status copyMessage(RawBusMessagePtr aSrc, RawBusMessagePtr& aDst, bool aIsAll) {
    if (!aSrc) {
        return Status(StatusCode::INVALID_ARG);
    }
...
RawMessageSharePtr::RawMessageSharePtr(RawBusMessagePtr aRawMsg, bool aIsOwned)
    : mRaw(aRawMsg)
    , mIsOwned(aIsOwned) {}

RawMessageSharePtr::~RawMessageSharePtr() {
    if (mIsOwned && mRaw) {
...
RawMessageSharePtr::RawMessageSharePtr(const RawMessageSharePtr& aPtr)
    : mRaw(aPtr.mRaw)
...
RawMessageSharePtr::RawMessageSharePtr(RawMessageSharePtr&& aPtr) noexcept
    : mRaw(aPtr.mRaw)
    , mIsOwned(aPtr.mIsOwned) {
    aPtr.mRaw = nullptr;
    aPtr.mIsOwned = false;
...
RawMessageSharePtr& RawMessageSharePtr::operator=(const RawMessageSharePtr& aPtr) {
    if (this == &aPtr) {
        return *this;
    }
...
RawMessageSharePtr& RawMessageSharePtr::operator=(RawMessageSharePtr&& aPtr) noexcept {
    if (this == &aPtr) {
        return *this;
    } 
...
RawMessageSharePtr RawMessageSharePtr::createReply(RawBusMessagePtr aCallMsg) {
    auto reply = Adaptor::RawMessage::createMethodReturn(aCallMsg);
    return RawMessageSharePtr(reply, true);
}

RawMessageSharePtr RawMessageSharePtr::createMethodCall(
